<template>
    <div class="shadow rounded p-3 bg-white">
        <div class="mb-3">
            <div class="d-flex justify-content-between">
                ...
                <h2 class="fw-lighter">{{ $t('serial.capacity') }}{{ this.deviceConfig_local.serial_config.length }} /
                    {{ this.deviceConfig_local.config_static.serial_max_count }}</h2>
            </div>
            <hr>
        </div>
        ...
        <div class="tab-content mb-5" id="pills-tabContent">
                ...
                <PerfectScaling scale="0.8">
                    <div class="table-responsive" style="max-height:75vh">
                        <table class="table table-striped table-hover text-center table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">
                                        ...
                                        <component class="text-primary" is="i-help" data-bs-toggle="tooltip"
                                            data-bs-html="true" :title="$t('serial.bar.hints.serialModel')" />
                                    </th>
                                    <!-- Modbus地址 -->
                                    <th scope="col">
                            ...
                            <tbody>
                                <tr v-for="(items, index) in getConfig.serial_config" :key="index" data-bs-toggle="tooltip"
                                :title="getConfig.SerialChannelReferencedCheck[index] ? $t('msg.delErrMsg_referenced') : ''">
                                    <!-- 序号 -->
                                    <td>{{ index + 1 }}</td>
                                    <!-- 启用 -->
                                    <td><input class="form-check-input fs-5" type="checkbox" v-model="items.serial_enable"
                                            @dblclick="dblClickToSelectAll()"
                                            :disabled="getConfig.SerialChannelReferencedCheck[index]">
                                        </td>
                                    <!-- 串口设备 -->
                                    <!-- <td>
                                        <select class="form-select" v-model="items.serial_device_path" style="min-width: 7em;">
                                            <option v-for="(item, index) in getTTY_dev" :key="index" :value="item.path" v-show="item.label!=''">
                                                {{ item.label }}
                                            </option>
                                        </select>
                                    </td> -->
                                    <!-- copilot 帮我限制一下下拉框的数量，只能选前serial_max_count个 -->
                                    <td>
                                        <select class="form-select" v-model="items.serial_device_path"
                                            style="min-width: 7em;"
                                            v-bind:class="{ 'is-invalid': getSerialConflictError(index) }"
                                            aria-describedby="serial_conflict_checking_hasError"
                                            :disabled="getConfig.SerialChannelReferencedCheck[index]">
                                            <option v-for="(item, index) in getTTY_dev" :key="index" :value="item.path"
                                                v-show="index < getConfig.config_static.serial_max_count">
                                                {{ item.label }}
                                            </option>
                                        </select>
                                        <div id="serial_conflict_checking_hasError" class="invalid-feedback">
                                    ...
                                    <td data-bs-html="true">
                                        <select class="form-select" v-model="items.serial_model" style="min-width: 9em;" :disabled="getConfig.SerialChannelReferencedCheck[index]">
                                    ...
                                    <td><input class="form-control" type="number" min="0" max="247"
                                            v-model="items.modbus_address" :disabled="items.serial_model != 2"
                                            style="min-width: 4em;"></td>
                                    <!-- Modbus超时时间(ms) -->
                                    <td><input class="form-control" type="number" min="0" max="65535"
                                            v-model="items.modbus_timeout" style="min-width: 6em;"></td>
                                    <!-- Modbus指令间隔(ms) -->
                                    <td><input class="form-control" type="number" min="0" max="65535"
                                            v-model="items.modbus_interval" style="min-width: 6em;"></td>
                                    <!-- 串口波特率 -->
                                    <td>
                                        <select class="form-select" v-model="items.serial_baudrate" style="min-width: 7em;">
                                    ...
                                    <!-- 数据位 -->
                                    <td>
                                        <select class="form-select" v-model="items.serial_bytesize" style="min-width: 4em;">
                                    ...
                                    <!-- 停止位 -->
                                    <td>
                                        <select class="form-select" v-model="items.serial_stopbits" style="min-width: 4em;">
                                    ...
                                    <!-- 校验位 -->
                                    <td>
                                        <select class="form-select" v-model="items.serial_parity" style="min-width: 6em;">
                                            <option value="0">{{ $t('serial.item.noneParity') }}</option>
                                            <option value="1">{{ $t('serial.item.oddParity') }}</option>
                                            <option value="2">{{ $t('serial.item.evenParity') }}</option>
                                        </select>
                                    </td>
                                    <!-- 网络模式 -->
                                    <td>
                                        <select class="form-select" v-model="items.network_config.network_model"
                                            style="min-width: 8em;">
                                            <option value="0">{{ $t('serial.item.tcpServer') }}</option>
                                            <option value="1" disabled>{{ $t('serial.item.tcpClient') }}</option>
                                            <option value="2" disabled>{{ $t('serial.item.udp') }}</option>
                                        </select>
                                    </td>
                                    <!-- 设备端口 -->
                                    ...
                                    <td><input class="form-control" type="text"
                                            ...
                                            :disabled="items.network_config.network_model == 0" style="min-width: 12em;">
                                    </td>
                                    <!-- 远程端口 -->
                                    <td><input class="form-control" type="number" min="500" max="65535"
                                        @blur="if(items.network_config.remote_port<500) items.network_config.remote_port=500;if(items.network_config.remote_port>65535) items.network_config.remote_port=65535;"
                                            ...
                                            :disabled="items.network_config.network_model == 0" style="min-width: 6em;">
                                    </td>
            ...
            <div class="tab-pane fade" id="pills-json-editor" role="tabpanel" aria-labelledby="pills-json-editor-tab">
                <div class="row">
                    <JsonEditorVue class="editor" language="zh-CN" v-model="getConfig.serial_config" />
                </div>
            </div>
        </div>
        ...
        <div class="mt-3 d-flex justify-content-start">
            <button type="button" class="btn btn-primary w-25 me-5" @click="add()">{{ $t('serial.bottom.add') }}</button>
            <button type="button" class="btn btn-warning w-25" @click="updateConfig()">{{ $t('serial.bottom.save')
            }}</button>
            <!-- 加载动画 -->
    ...
    <!-- 模态框 -->
    <Modal ref="modalRef" :title="$t('serial.modal.title')" mode="OK" :close_handle="modal_close_handle"></Modal>
</template>
    ...
    methods: {
        dblClickToSelectAll() {
            ...
            for (var i = 0; i < this.deviceConfig_local.serial_config.length; i++) {
                ...
                this.deviceConfig_local.serial_config[i].serial_enable = this.isDblClickToSelectAll;
            }
        },
        modal_close_handle() {
            //刷新页面
        ...
        add() {
            ...
            if (this.deviceConfig_local.serial_config.length >= maxlength) {
                alert(this.$t('serial.msg.addErrMsg') + maxlength + this.$t('serial.msg.addErrMsgUnit'));
                return;
            }
            // 添加新的一行
            let newRow = { "serial_enable": false, "serial_device_path": "/dev/ttyS1", "serial_model": 0, "modbus_address": 0, "modbus_timeout": 1000, "modbus_interval": 1000, "serial_baudrate": 7, "serial_bytesize": 1, "serial_stopbits": 0, "serial_parity": 0, "network_config": { "network_model": 0, "local_port": 8080, "remote_address": "remote.com", "remote_port": 2048 } };
        ...
        getSerialConflictError(index) {
            ...
            let serial_device_path = this.deviceConfig_local.serial_config[index].serial_device_path;
            let serial_enable = this.deviceConfig_local.serial_config[index].serial_enable;
            ...
            for (let i = 0; i < this.deviceConfig_local.serial_config.length; i++) {
                ...
                if (this.deviceConfig_local.serial_config[i].serial_device_path == serial_device_path) {
                    //同时禁用当前的通道
    ...
    computed: {
        ...
        getTTY_dev() {
            ...
            // });
                ...
                let processedTTY_dev = JSON.parse(JSON.stringify(this.$store.state.TTY_dev));
                processedTTY_dev.forEach(device => {
